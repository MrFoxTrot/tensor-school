    ...
    while True:
        ...
        if i.lower() == "цвета":
            ...
            if code not in colors.keys():
                ...
                print(
                    ("\n[✔] Информация о цвете:\nКод: %s" +
                     "\nКод в 10-чной системе: %s;" +
                     " \t в 16-чной системе: #%02x%02x%02x\n")
                    % (code, color, *color))
        elif i.lower() == "стоп":
            break
        else:
...
def task_3():
    ...
    print(" Входящих в первое или во второе," +
          " но не в оба из них одновременно: %s " %
          list(set(array1) ^ set(array2)))


def task_4():
    """Создать игровой инвентарь.
    Должна быть возможность добавлять/удалять предметы предметы из него.
    ...
    while True:
        ...
        print(" удалить [номер предмета в инвентаре]" +
              " - удалить предмет из инветаря")
        print(" стоп - выйти")
        i = input(": ")
        if i.lower() == "инвентарь":
            ...
            print(
                "Список предметов:\n\tНазвание\tВес\n%s" % '\n'.join(
                    ["%d)%s -\t%s\t\t%0.2f" %
                     (i + 1, items[i].emoji, items[i].name, items[i].weight)
            ...
            try:
                ...
                print("[❌] Неверно указан номер предмета" +
                      " (укажите номер числом)")
            except:
                print("[❌] Неверно указан номер предмета " +
                      "(предмета с таким номером нет)")
            ...
            try:
                ...
                if item_id < 0 or len(items) < item_id:
                    return print("[❌] Неверно указан номер предмета" +
                                 "(предмета с таким номером нет)")
