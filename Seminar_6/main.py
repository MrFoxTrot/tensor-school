...
def task_1():
    ...
    print(
        "Введите строки для перевода в байт код" +
        "(напишите 'стоп' для окончания вводе)"
    )
    input_strings = []
    while True:
        string = input(": ")
...
def task_2():
    """Формула молекулы спирта - C2H5(OH). Из неё видно,
    что молекула состоит из двух атомов углерода (С),
    ...
    try:
        with open("./input.txt", 'r') as file_input:
            ...
            try:
                ...
                return print("Не удалось выполнить запись ответа" +
                             " в файл output.txt. Файл не доступен")

            print("Максимально возможное кол-во" +
                  f" молекул спирта из {c=} {h=} {o=} = {res[0]}")
...
def task_3():
    ...
    def xor(text, secret):
        """Зашифровать/расшировать текст

        Аргументы:
        text   -- текст для преобразования
        sercet -- ключ шифрования
        """
        return ''.join(chr(ord(x) ^ ord(y)) for x, y in zip(
            text, cycle(secret)))
    ...
    try:
        with open("./in_cipher.txt", 'r') as file_input:
            ...
            try:
                ...
                return print("Не удалось выполнить запись ответа" +
                             " в файл out_cipher.txt. Файл не доступен")
    except IOError:
